...
const App: React.FC = () => {
  ...
  const preflightKeyCheck = async (): Promise<void> => {
    ...
    if (aistudio) {
      try {
        ...
        console.warn("Key preflight failed, proceeding with env key.");
      }
    }
  };
  ...
  const handleFormSubmit = async (newPrefs: UserPreferences) => {
    ...
    try {
      ...
      if (msg.includes("500") || msg.includes("UNKNOWN") || msg.includes("key") || msg.includes("403")) {
        setError("AUTHENTICATION ERROR: The architectural engine requires a valid Gemini API Key. If you are on Vercel, please ensure your API_KEY is set in environment variables.");
  ...
  const handleProjectSelect = async (summary: ProjectSummary) => {
    ...
    try {
      ...
      setError("Failed to initialize project blueprint. Please verify your connection and try again.");
    } finally {
  ...
  return (
    <div className="premium-bg selection:bg-orange-500/30 selection:text-white flex flex-col min-h-screen">
      ...
      <div className="relative z-10 flex flex-col flex-grow">
        ...
        <main className="container mx-auto px-6 pb-40 flex-grow">
          {error && (
            <div className="max-w-4xl mx-auto mb-16 p-8 bg-rose-500/10 border border-rose-500/30 text-rose-400 rounded-3xl text-center font-bold animate-in slide-in-from-top-4 backdrop-blur-xl">
              ...
              <button onClick={() => (window as any).aistudio?.openSelectKey()} className="px-6 py-2 bg-rose-500 text-white rounded-full text-xs font-black uppercase tracking-widest">Re-Select Key</button>
            </div>
          )}
          ...
          {view === AppView.FORM && (
            <div className="max-w-4xl mx-auto bg-[#0a0a0a]/80 backdrop-blur-3xl rounded-[3.5rem] p-10 sm:p-20 border border-white/10 animate-in zoom-in-95">
              <div className="mb-16 text-center">
                ...
                <p className="text-slate-400 font-medium">Define your academic roadmap goals.</p>
              </div>
        ...
        {isLoadingDetail && (
          <div className="fixed inset-0 bg-black/90 z-[10000] flex items-center justify-center p-6">
            <div className="text-center">
              <div className="w-16 h-16 border-4 border-orange-500 border-t-transparent rounded-full animate-spin mx-auto mb-8"></div>
